...
void LCD_Init(void)
{
	...
	DIO_vSetPortHighNibbleDir(LCD_PORT,OUTPUT);
	DIO_vSetPinDir(RS_PORT,RS_PIN,OUTPUT);
	DIO_vSetPinDir(EN_PORT,EN_PIN,OUTPUT);
...
void LCD_WriteCommand(u8 Command)
{
	...
	_delay_ms(1);
	#elif defined FOUR_BIT_MODE
	DIO_vSetPortHighNibbleValue(LCD_PORT,(Command>>4));
	DIO_vSetPinValue(RS_PORT,RS_PIN,LOW);
	Enable();
	DIO_vSetPortHighNibbleValue(LCD_PORT,Command);
	DIO_vSetPinValue(RS_PORT,RS_PIN,LOW);
	Enable();
	_delay_ms(1);
	#endif
...
void LCD_WriteChar(u8 character)
{
	...
	_delay_ms(1);
	#elif defined FOUR_BIT_MODE
	DIO_vSetPortHighNibbleValue(LCD_PORT,(character>>4));
	DIO_vSetPinValue(RS_PORT,RS_PIN,HIGH);
	Enable();
	DIO_vSetPortHighNibbleValue(LCD_PORT,character);
	DIO_vSetPinValue(RS_PORT,RS_PIN,HIGH);
	Enable();
	_delay_ms(1);
	#endif
